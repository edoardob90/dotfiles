...
# Last Modified: 4 April 2017 @ 17:23 by Edoardo Baldi
...
function load_prompt () {
    # Get PIDs
    local parent_process=$(< /proc/$PPID/cmdline tr -d \\0 | cut -d'.' -f 1) # Modified to avoid null byte in input (EB)
    local my_process=$(< /proc/$$/cmdline tr -d \\0 | cut -d'.' -f 1)
